...
//     }); 
    ...
    queryParam = new SuperMap.REST.FilterParameter({ 
        name: "Puff_Point@SDDY_GIS", 
        attributeFilter: "Value =1 " ,
        fields: ["SmID","SMX","SMY","Value"]
     }), 
    ...
    queryBySQLService = new SuperMap.REST.QueryBySQLService(url, { 
        eventListeners: { 
            "processCompleted": puffCompleted, 
            "processFailed": puffFailed 
            } 
    }); 
    ...
    queryBySQLService1 = new SuperMap.REST.QueryBySQLService(url, { 
        eventListeners: { 
            "processCompleted": puffCompleted1, 
            "processFailed": puffFailed1 
            } 
    }); 
    ...
    queryBySQLService2 = new SuperMap.REST.QueryBySQLService(url, { 
        eventListeners: { 
            "processCompleted": puffCompleted2, 
            "processFailed": puffFailed2 
            } 
    }); 
    queryBySQLService2.processAsync(queryBySQLParams2); 
	}
	function puffCompleted(queryEventArgs) {
			...
			if (result && result.totalCount>0) {
					...
					for	(var i = 0;	i < result.recordsets.length; i++) {											
						if (result.recordsets[i].features) {
							...
							for(var	j=result.recordsets[i].features.length-1;j>0;j--){
								var	feature1= new SuperMap.Feature.Vector();
								feature1	= result.recordsets[i].features[j];
								var pointx1 = feature1.attributes["SMX"],
								pointy1=feature1.attributes["SMY"];
                                var point1=new SuperMap.Geometry.Point(pointx1,pointy1);
                                if(j%2!=0)
                                 linearRings.addComponent(point1);
                                
							}
						}
					}
								var region = new SuperMap.Geometry.Polygon([linearRings]);
								var	featurep= new SuperMap.Feature.Vector(linearRings,null,styleyellow);
								vectorLayer.addFeatures(featurep);
				}
				else{
		...
		function puffFailed(e) {
		alert(e.error.errorMsg);
		}
		function puffCompleted1(queryEventArgs) {
 			 var result = queryEventArgs.result;
			//显示查找到的矢量要素
			if (result && result.totalCount>0) {
					var multiPoint=new SuperMap.Geometry.MultiPoint();
					...
					for	(var i = 0;	i < result.recordsets.length; i++) {											
						if (result.recordsets[i].features) {
							...
							//alert(linearRings);
								var region = new SuperMap.Geometry.Polygon([linearRings]);
								var	featurep= new SuperMap.Feature.Vector(linearRings,null,styleorange);
								//alert(featurep);
								vectorLayer.addFeatures(featurep);
				}
				else{
		...
		function puffFailed1(e) {
		alert(e.error.errorMsg);
		}
		function puffCompleted2(queryEventArgs) {
 			 var result = queryEventArgs.result;
			//显示查找到的矢量要素
			if (result && result.totalCount>0) {
					var multiPoint=new SuperMap.Geometry.MultiPoint();
					...
					for	(var i = 0;	i < result.recordsets.length; i++) {	
						if (result.recordsets[i].features) {
							...
							//	var region = new SuperMap.Geometry.Polygon([linearRings]);
								var	featurep= new SuperMap.Feature.Vector(linearRings,null,stylered);
								vectorLayer.addFeatures(featurep);
				}
				else{
							resultTable = "<p>无查询结果！</p>";
